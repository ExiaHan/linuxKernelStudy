...
##总结
通过上述分析，可以看到现代计算机在运行过程中由于要支持多任务，必不可少的要进行进程调度，进程调度中最为奇妙的就是上下文的转换，这里我们看到，**对于mykernel**来说，next和prev之间的角色是相互转换的，上一个next在下一次进程调度的时候可能会变成prev，上一个prev在下一个进程调度的时候就是next。即是一种“参数化上下文转换”的形式。my_schedule方法是调度中的关键，其最奇妙的地方就是其执行总是分为两部分，调度时执行前一部分，下一次调度回来后才会执行剩下的部分。对于现代计算机来说，其多任务运行的方式就是上述过程的复杂化实现。